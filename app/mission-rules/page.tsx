      ...
      <div className="container">
        ...
        <div className="rules-section">
          ...
          {isLoading ? (
            ...
            <div className="rules-container">
              ...
              {/* Standard Rules - Redesigned for Readability */}
              {rules.filter(rule => !('isHero' in rule && rule.isHero) && !('isEmotional' in rule && rule.isEmotional) && !('isRemember' in rule && rule.isRemember)).length > 0 && (
                <div className="operational-protocols-section">
                  <div className={`${jetbrainsMono.className} protocols-header`}>
                    <h2>OPERATIONAL PROTOCOLS</h2>
                    <div className="protocols-subtitle">Mission-critical design and execution guidelines</div>
                  </div>
                  
                  <div className="protocols-container">
                    {rules.filter(rule => !('isHero' in rule && rule.isHero) && !('isEmotional' in rule && rule.isEmotional) && !('isRemember' in rule && rule.isRemember)).map((rule, index) => {
                      const ruleLevel = rule.level || 'STANDARD'
                      const ruleId = rule.id || `PROTOCOL-${String(index + 1).padStart(2, '0')}`
                      const hasIndex = 'index' in rule && typeof rule.index === 'number'
                      const sectionIndex = hasIndex ? rule.index : index
                      
                      // Parse content for better structure
                      const content = rule.content || ''
                      const lines = content.split('\n').filter(line => line.trim())
                      const mainContent = lines.find(line => !line.startsWith('**') && !line.startsWith('#') && line.length > 50) || content
                      const keyPoints = lines.filter(line => line.startsWith('- ') || line.startsWith('* '))
                      
                      return (
                        <div key={ruleId} className={`protocol-card ${ruleLevel.toLowerCase()}`}>
                          <div className="protocol-header">
                            <div className="protocol-meta">
                              <div className={`${jetbrainsMono.className} protocol-id`}>{ruleId}</div>
                              <div className={`protocol-priority ${ruleLevel.toLowerCase()}`}>
                                {ruleLevel === 'CRITICAL' ? '🔴 CRITICAL' : '🟡 STANDARD'}
                              </div>
                            </div>
                            <div className={`${jetbrainsMono.className} protocol-category`}>
                              {rule.category || 'GENERAL PROTOCOL'}
                            </div>
                          </div>
                          
                          <div className="protocol-title">
                            {rule.title || 'Untitled Protocol'}
                          </div>
                          
                          <div className="protocol-content">
                            <div className="protocol-description">
                              {mainContent.replace(/\*\*/g, '').replace(/#{1,3}\s*/g, '').trim()}
                            </div>
                            
                            {keyPoints.length > 0 && (
                              <div className="protocol-points">
                                <div className={`${jetbrainsMono.className} points-label`}>Key Requirements:</div>
                                <ul className="points-list">
                                  {keyPoints.slice(0, 3).map((point, idx) => (
                                    <li key={idx} className="protocol-point">
                                      {point.replace(/^[-*]\s*/, '')}
                                    </li>
                                  ))}
                                </ul>
                          ...
                          <div className="protocol-footer">
                            <div className={`${jetbrainsMono.className} protocol-status`}>
                              <span className="status-indicator">●</span>
                              ENFORCED
                            </div>
                            {hasIndex && (
                              <div className={`${jetbrainsMono.className} protocol-section`}>
                                SEC {sectionIndex + 1}
                              </div>
                            )}
                          </div>
        ...
        <div className="compliance-section">
          <div className="compliance-header">
            <div className={`${jetbrainsMono.className} compliance-title`}>COMPLIANCE VERIFICATION</div>
          </div>
          <div className="compliance-content">
            <p>All design implementations must undergo protocol verification before deployment. 
            Non-compliance with CRITICAL protocols will result in mission failure. 
            STANDARD protocols ensure optimal user experience and operational efficiency.</p>
            
            <div className="compliance-checklist">
              <div className={`${jetbrainsMono.className} checklist-title`}>Pre-Deployment Checklist:</div>
              <ul>
                <li>✓ Visual hierarchy protocols verified</li>
                <li>✓ Typography standards implemented</li>
                <li>✓ Interaction feedback confirmed</li>
                <li>✓ Data presentation optimized</li>
                <li>✓ Navigation flow tested</li>
                <li>✓ Responsive deployment ready</li>
              </ul>
            </div>
          </div>
        </div>
